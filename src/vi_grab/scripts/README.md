...
# 6.train model:
1.将你标注好的数据集放入vi_grab/scripts/yolov5/shujuji文件夹中,格式如同示例一样，分为rrain和val两个文件夹，文件夹中分别存储图片和其对应的标签

2.在vi_grab/scripts/下打开终端运行 "python3 train.py --img 640 --batch 1 --epochs 10 --data ./data/test.yaml --cfg ./models/yolov5s_test.yaml --weights '' --device cpu",你可以替换-batch 1 --epochs 10的值，以适配你自己的训练要求
  
3.训练结果存储在 runs/train/,复制 best.pt model 到 vi_grab/scripts/weights/, 创建一个yaml文件如示例文件一样,只需要更改类别数量和类别名称.

4.更改运行代码中  model = YoloV5(yolov5_yaml_path='src/vi_grab/scripts/config/yolov5s.yaml')的yolov5s.yaml为你创建的yaml文件名字.

5.正常运行检测代码即可查看自己训练模型的检测效果
