...
class YoloV5:
    def __init__(self, yolov5_yaml_path='src/vi_grab/scripts/config/yolov5s.yaml'):
        '''初始化'''
        # 载入配置文件
...
if __name__ == '__main__':
    ...
    model = YoloV5(yolov5_yaml_path='src/vi_grab/scripts/config/yolov5s.yaml')
    ...
    try:
        while True:
            ...
            if xyxy_list:
                ...
                #print(class_id_list,camera_xyz_list)
                for id in class_id_list:
                    object_info_msg.object_class = str(model.yolov5['class_name'][id])
                    object_info_msg.x = float(camera_xyz_list[i][0])
                    object_info_msg.y = float(camera_xyz_list[i][1])
                    object_info_msg.z = float(camera_xyz_list[i][2])
                    print(object_info_msg)
